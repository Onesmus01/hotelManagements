...
const SearchBar = () => {
  ...
  const handleClear = () => {
    setDestination("");
    setCheckIn(new Date());
    setCheckOut(new Date(Date.now() + 86400000)); // Reset to tomorrow
    setAdultCount(1);
    setChildCount(0);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1">
        <button
          type="submit"
          className="w-2/3 bg-blue-600 text-white h-full font-bold text-xl hover:bg-blue-500"
        >
          Search
        </button>
        <button
          type="button"
          className="w-1/3 bg-red-600 text-white h-full font-bold text-xl hover:bg-red-500"
          onClick={handleClear}
        >
          Clear
        </button>
